...
namespace eSyaNatureCure.DL.Repository
{
    public class GuestCheckOutRepository : IGuestCheckOutRepository
    {
            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            return new DO_ReturnParameter { Status = false, Message = "Bill has been generated for this booking id. Do not have permission to modify the bill." };
                        }

                        if (obj.UHID <= 0)
                        {
                            return new DO_ReturnParameter { Status = false, Message = "Guest has to be checked-in first." };
                        }
            ...
            using (var db = new eSyaEnterprise())
            {
                var gd = await db.GtGptbkh
                    ...
                    .Select(r => new DO_PatientBillDetails
                    {
                        ...
                        Quantity = 1,
                        Rate = r.g.TotalPackageAmount,
                        DiscountAmount = r.g.TotalDiscountAmount,
                        TotalAmount = r.g.NetPackageAmount
                    })
                    .ToListAsync();
        ...
        public async Task<DO_PatientBillHeader> GetGuestBillingSummary(int businessKey, long bookingKey)
        {
            using (var db = new eSyaEnterprise())
            {
                var pk = await db.GtGptbkh.Where(w => w.BusinessKey == businessKey && w.BookingKey == bookingKey)
                    .FirstOrDefaultAsync();
                var packageAmount = pk.NetPackageAmount;

                var gh = await db.GtGptbkh.Where(w => w.BusinessKey == businessKey && w.BookingKey == bookingKey && w.BillDocumentKey != null).FirstOrDefaultAsync();
                if (gh != null)
                {
                    var ph = await db.GtEfpbhd
                       .Where(w => w.BusinessKey == businessKey && w.BillDocumentKey == gh.BillDocumentKey)
                       .Select(r => new DO_PatientBillHeader
                       {
                           BillDocumentKey = r.BillDocumentKey,
                ...
                return new DO_PatientBillHeader()
                {
                    PackageAmount = packageAmount,
                    NetBillAmount = packageAmount
                };
            }
        }
            ...
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            return new DO_ReturnParameter { Status = false, Message = "Bill has not been generated for this booking id." };
                        }
                        ...
                        var rg = await db.GtGptbkg.Where(w => w.BusinessKey == obj.BusinessKey
                                    && w.BookingKey == obj.BookingKey
                                    && !w.IsCheckedOut).ToListAsync();

                        if (rg.Count() <= 0)
                        {
                            return new DO_ReturnParameter { Status = false, Message = "Already Checkout process has been done." };
                        }

                        foreach (var g in rg)
                        {
                            g.IsCheckedOut = true;
                            g.ActualCheckedOutDate = DateTime.Now;
                            g.ModifiedBy = obj.UserID;
                            g.ModifiedOn = DateTime.Now;
        ...
        public async Task<DO_ReturnParameter> GetGuestCheckOutStatus(int businessKey, long bookingKey, int guestId)
        {
            using (var db = new eSyaEnterprise())
            {
                var rg = await db.GtGptbkg.Where(w => w.BusinessKey == businessKey
                            && w.BookingKey == bookingKey
                            && w.GuestId == guestId).FirstOrDefaultAsync();
        ...
        public async Task<DO_ReturnParameter> GuestBookingCancellationProcess(DO_GuestCheckInDetails obj)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        var bk = await db.GtGptbkh.Where(w => w.BusinessKey == obj.BusinessKey && w.ActiveStatus && w.BookingKey == obj.BookingKey).FirstOrDefaultAsync();
                        if (bk != null)
                        {
                            var ph = await db.GtEfpbhd.Where(w => w.BusinessKey == obj.BusinessKey && w.BillDocumentKey == bk.BillDocumentKey && w.ActiveStatus).FirstOrDefaultAsync();
                            ...
                            if (ph != null)
                            {
                                return new DO_ReturnParameter { Status = false, Message = "Bill has been generated." };
                            }
                        }
                        ...
                        if (p_collectedAmount + p_RefundAmount > 0)
                        {
                            return new DO_ReturnParameter { Status = false, Message = "Payment has been made for this booking.Cannot cancel this booking" };
                        }
                        ...
                        if (bk != null)
                        {
                            bk.ReasonforCancellation = obj.ReasonforCancellation;
                            bk.ActiveStatus = false;
                            bk.ModifiedBy = obj.UserID;
                            bk.ModifiedOn = System.DateTime.Now;
                        ...
                        {
                            return new DO_ReturnParameter { Status = false, Message = "Booking Header with This Booking Key is In-Active." };
