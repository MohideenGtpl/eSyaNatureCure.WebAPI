   ...
   public class RoomReservationRepository: IRoomReservationRepository
    {
        ...
        public async Task<List<DO_RoomReservation>> GetRoomReservationsbyBusinesskey(int businesskey, int roomtypeId,string roomNumber)
        {
            using (var db = new eSyaEnterprise())
            {
                try
                {
                    ...
                    var result = db.GtGprmrv.Where(x=>x.TillDate.Date >= DateTime.Now.Date).Join(db.GtEcapcd,
                           lkey => new { lkey.ReasonType },
                           ent => new { ReasonType = ent.ApplicationCode },
                           (lkey, ent) => new { lkey, ent }).Join(db.GtGprmty,
                           Bloc => new { Bloc.lkey.RoomTypeId },
                           ...
                           (bm, bme) => new { bm, bme }).Where(x=>x.bm.Bloc.lkey.BusinessKey==businesskey
                            && (roomtypeId == 0 || x.bm.Bloc.lkey.RoomTypeId == roomtypeId)
                            && (roomNumber == "0" || x.bm.Bloc.lkey.RoomNumber == roomNumber))
            ...
            using (eSyaEnterprise db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        var _isbooked = db.GtGptbkh
                            .Join(db.GtGptbkp.Where(x=>x.ActiveStatus==true).Where(x=>x.RoomTypeId == obj.RoomTypeId 
                                && x.RoomNumber == obj.RoomNumber 
                                && x.BedNumber == obj.BedNumber 
                                && x.ActiveStatus),
                        h => new { h.BusinessKey, h.BookingKey },
                        p => new { p.BusinessKey, p.BookingKey },
                       (h, p) => new { h, p })
                       .Where( w => 
                            ((obj.EffectiveDate.Date == w.h.CheckInDate)
                           || (obj.EffectiveDate.Date == w.h.CheckOutDate.Date)
                           || (obj.EffectiveDate.Date < w.h.CheckInDate.Date && obj.TillDate.Date >= w.h.CheckInDate.Date)
                           || (obj.EffectiveDate.Date > w.h.CheckInDate.Date && obj.EffectiveDate.Date <= w.h.CheckOutDate.Date)))
                       .Select(r => new DO_GuestCheckInDetails
                       {
                         CheckedInDate = r.h.CheckInDate,
                         CheckedOutDate = r.h.CheckOutDate,
                         BusinessKey = r.h.BusinessKey,
                        ...
                        if (_isbooked > 0)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Room is already Booked" };
                        }
                        if (is_rsExists == null)
                        {
        ...
        public async Task<List<DO_BedMaster>> GetActiveRoomNumbers()
        {
            using (var db = new eSyaEnterprise())
            {
                try
                {
                    var ds = await db.GtGprmbm.Where(x => x.ActiveStatus == true)
                        .Select(r => new DO_BedMaster
                        {
                            RoomNumber = r.RoomNumber
